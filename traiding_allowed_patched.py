    ...
    try:
        ...
        for i, (symbol, side, price, amount) in enumerate(steps, 1):
            try:
                ...
                print(f"🔹 Ордер {i}: {side.upper()} {amount:.6f} {symbol} по {rounded_price:.6f}")
                ...
                for o in executed_orders:
                    try: await exchange.cancel_order(o['id'], o['symbol'])
                    except: pass
                return False
...
async def check_triangle(base, mid1, mid2, symbols, markets):
    try:
        ...
        print(f"🧠 Арбитраж найден: {route_id} | Спред: {profit_percent:.2f}%")
        await send_telegram_message(f"🧠 Арбитраж: {route_id}\nСпред: {profit_percent:.2f}%\nОбъем: {dynamic_volume:.2f} USDT")
        await execute_real_trade(route_id, trade_steps, base_coin, markets, dynamic_volume)
    except Exception as e:
