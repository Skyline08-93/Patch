    ...
    try:
        ...
        for i, (symbol, side, price, amount) in enumerate(steps, 1):
            try:
                ...
                amount_precision = int(market['precision']['amount'])  # Преобразуем в int
                price_precision = int(market['precision']['price'])    # Преобразуем в int
                ...
                # Корректное округление цены
                rounded_price = round(float(price) / (10 ** -price_precision)) * (10 ** -price_precision)
                
                print(f"🔹 Ордер {i}: {side.upper()} {adjusted_amount} {symbol} @ {rounded_price}")
                order = await exchange.create_order(
                    symbol=symbol,
                    type='limit',
                    side=side,
                    amount=adjusted_amount,  # Уже округленное значение
                    price=rounded_price,    # Уже округленное значение
                    params={'timeInForce': 'PostOnly'}
                )
                executed_orders.append(order)
                await asyncio.sleep(3)
                
                # Получаем статус ордера через закрытые ордера
                closed_orders = await exchange.fetchClosedOrders(symbol, limit=10)
                order_status = next((o for o in closed_orders if o['id'] == order['id']), None)
                
                if not order_status:
                    # Проверяем в открытых ордерах на случай, если он еще не исполнился
