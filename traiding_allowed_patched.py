    ...
    try:
        ...
        for i, (symbol, side, price, amount) in enumerate(steps, 1):
            try:
                ...
                # Округление объема до точности рынка
                amount_precision = market['precision']['amount']
                adjusted_amount = round(float(amount), amount_precision)
                
                # Проверка минимального объема для конкретной пары
                min_amount = market['limits']['amount']['min']
                if adjusted_amount < min_amount:
                    msg = f"❌ Объем {adjusted_amount} {symbol.split('/')[0]} меньше минимума {min_amount}"
                    print(msg)
                ...
                print(f"🔹 Ордер {i}: {side.upper()} {adjusted_amount} {symbol} по {rounded_price}")
                ...
                # Исправление для UTA: используем fetchClosedOrders
                closed_orders = await exchange.fetchClosedOrders(symbol, since=None, limit=1)
                order_status = next((o for o in closed_orders if o['id'] == order['id']), None)
                
                if not order_status:
                    raise ValueError(f"❌ Ордер {order['id']} не найден в истории")
                ...
                await send_telegram_message(f"🔥 Ошибка шага {i}: {e}")
