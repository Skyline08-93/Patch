...
triangle_cache = {}
min_order_value = 1.0  # Минимальная сумма ордера в USDT
...
async def get_available_balance(coin='USDT'):
    try:
        ...
        return float(balance['total'].get(coin, {}).get('totalBalance', 0))
    except Exception as e:
...
async def get_execution_price(symbol, side, target_usdt):
    try:
        ...
        if not side_data or len(side_data) == 0:
            print(f"❌ Пустой стакан для {symbol} {side}")
            return None, 0, 0
        ...
        for price, volume in side_data:
            try:
                ...
                if total_usd + usd >= target_usdt:
                    remaining_usd = target_usdt - total_usd
                    total_base += remaining_usd / price
                ...
                print(f"⚠️ Ошибка обработки уровня стакана {symbol}: {e}")
                continue

        if total_usd <= 0 or total_base <= 0:
            print(f"❌ Недостаточная ликвидность для {symbol} {side}")
            return None, 0, 0

        avg_price = total_usd / total_base
        return avg_price, total_usd, total_usd
